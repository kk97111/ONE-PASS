...
import sys
import time
from utils import GenerationDataset, combined_collate_fn ,rank_net
...
class FullFinetuneRecipeDistributed(FTRecipeInterface):
    ...
    def setup(self, cfg: DictConfig) -> None:
        ...
        tokenizer_path = cfg.backbone_path
    ...
    def _setup_data(
        ...
        ds = load_from_disk(cur_dir + "/efficient_inference/dataset/rank_zephyr_IR/%s"%self.backbone)
        size = ds.num_rows
        train_index = int(size * 0.8)
        valid_index = int(size * 0.1)
        valid_index = train_index + valid_index

        train_data = ds.select(range(train_index))
        valid_data = ds.select(range(train_index, valid_index))
        test_data = ds.select(range(valid_index, size))


        raw_train_data = train_data
        train_dataset = GenerationDataset(raw_train_data, self._tokenizer, combined=True)
        train_collate_fn = partial(combined_collate_fn, tokenizer=self._tokenizer)
        self._tokenizer.pad_token = self._tokenizer.eos_token
        self._tokenizer.pad_token_id = self._tokenizer.eos_token_id = self._tokenizer.convert_tokens_to_ids(self._tokenizer.eos_token)
        ...
        dataloader = DataLoader(
            ...
            drop_last=True,
            collate_fn=train_collate_fn  # 这里使用 combined_collate_fn
    ...
    def save_checkpoint(
        ...
        torch.distributed.barrier()
    def _loss_step(self, batch: Dict[str, torch.Tensor],labels: torch.Tensor) -> torch.Tensor:
        ...
        return loss
    def _loss_step_rank(self, batch: Dict[str, torch.Tensor],labels: torch.Tensor,rank_labels:list) -> torch.Tensor:
        START_IDX = ord('A')
        tokens_ids = batch["tokens"] 
        with self.activations_handling_ctx:
            logits = self._model(**batch)
            logits = torch.concat(logits,dim=1)
        rank_losses = torch.zeros(len(rank_labels)).to(self._device)
        for batch_index, true_rank in enumerate(rank_labels):
            pred_logits = logits[batch_index]
            rank_pred_index = (labels[batch_index] != -100).nonzero(as_tuple=True)[0][0] - 1
            pred = pred_logits[rank_pred_index]
            gather_indices = [self._tokenizer.convert_tokens_to_ids(chr(c)) for c in range(START_IDX, START_IDX+len(true_rank))]
            scores = torch.gather(pred, 0, torch.tensor(gather_indices).to(self._device))
            scores_sorted, _ = scores.sort(descending=True, dim=-1)
            true_scores = torch.gather(scores_sorted, 0, torch.tensor(true_rank).to(self._device))
            assert true_scores.shape == true_scores.shape,"维度对不齐！！"


            
            rank_losses[batch_index] = nn.MSELoss()(scores.unsqueeze(0), true_scores.unsqueeze(0)) 
            
            # rank_net(scores.unsqueeze(0), true_scores.unsqueeze(0), weighted='store_true',cuda = self._device)
        # 计算每个token的损失
        rank_loss = rank_losses.mean()
        if not torch.isnan(rank_loss):
            rank_loss = torch.mul(rank_loss, 10.0)  
        del logits
        return rank_loss
    def train(self,cfg: DictConfig) -> None:
        ...
        for curr_epoch in range(self.epochs_run, self.total_epochs):
            ...
            for idx, batch_ts in enumerate(self._dataloader):
                if (
                    self.max_steps_per_epoch is not None
                ...
                batch = dict()
                batch['tokens'] = batch_ts[0].to(self._device)
                labels = batch_ts[1].to(self._device)
                rank_labels = batch_ts[2]
                # Calculate the number of unmasked tokens in the current batch
                # and increment the total number of tokens seen in the step
                current_num_tokens = (
                   labels != self._loss_fn.ignore_index
                ).sum()
                num_tokens += current_num_tokens

                loss_SFT = self._loss_step(batch,labels)
                if cfg.RankNet:
                    loss_rank = self._loss_step_rank(batch,labels,rank_labels)
                else:
                    loss_rank = 0
                current_loss = (loss_SFT + loss_rank) * current_num_tokens
